...
node () {
   ...
   stage('Build') {
      ...
      try{
        if (isUnix()) {
          configFileProvider([configFile(fileId: 'e1196efe-98a0-4f47-b2ad-358cbb79ccad', variable: 'MY_SETTINGS_XML')]) {
           sh "./mvnw -B -s $MY_SETTINGS_XML -Dmaven.test.failure.ignore -Drat.skip=true -f pom.xml clean package -U"
          }
        } else {
